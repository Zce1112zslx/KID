...
def evaluate(opt, model, dataset, epoch_num, best_metrics=None, show_case=False):
    model.eval()
    print('begin eval')
    ...
    with torch.no_grad():
        ...
        if best_metrics:
            ...
            if metrics[-1] > best_metrics:
                ...
                torch.save(model.state_dict(), opt.save_model_path + "_" + str(epoch_num) + ".bin")
...
if __name__ == '__main__':
    ...
    if opt.mode == 'train':
        train_dataset = FrameNetDataset(opt, config, config.exemplar_instance_dic, device)
        ...
        for epoch in range(epochs):
            ...
            if epoch == 0 or epoch % 10 == 9:
                best_metric = evaluate(opt, model, train_dataset, epoch+1, -1)
            else:
                evaluate(opt, model, train_dataset, epoch+1)
